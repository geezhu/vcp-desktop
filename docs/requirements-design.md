...
8. 本阶段仅定义 Linux 安装闭环（目标 `x86_64`，Ubuntu/Debian 作为首轮回归基线）。
9. 支持“首次初始化向导 + 后续点击 AppImage 一键启动后端与 VCPChat”的运行入口能力。
10. 默认采用“初始化下载私有运行时（Portable）”策略，不将依赖安装到系统全局。
...
1. 首次运行安装器时，若未发现初始化配置，必须进入初始化向导并保存启动配置。
2. 初始化配置至少包含：工作区路径、后端启动命令/目录、VCPChat 启动命令/目录、启动顺序与等待策略。
3. 初始化完成后，安装器可在同一次运行中继续启动后端与 VCPChat（可配置为仅初始化不启动）。
4. 后续运行安装器时，若已存在初始化配置，应默认跳过向导并直接执行一键启动流程。
5. 提供显式控制命令：`init`、`start`、`stop`、`status`、`reset`。
6. 启动流程应持久化运行状态（PID/日志路径）并支持状态查询与停止。

### FR-010 初始化运行时自举（Portable 模式）

1. 在 `init` 阶段支持下载并准备私有运行时（如 Node/Python 及其依赖），默认写入用户隔离目录。
2. 私有运行时目录应与项目配置/状态分离，并支持缓存复用与版本记录。
3. 下载产物必须具备完整性校验（至少 SHA256），校验失败不得进入后续安装步骤。
4. 安装器需支持断点续传/继续（`resume`）并避免重复下载已校验通过的产物。
5. 启动阶段应优先使用私有运行时（通过 wrapper 注入 `PATH`/`LD_LIBRARY_PATH`），默认不写系统级 PATH。
6. 除非用户显式启用系统集成模式，安装器不得调用系统包管理器执行全局安装。
...
1. 当前阶段目标平台：Linux `x86_64`（`glibc` 基线）。  
2. Ubuntu 22.04+ / Debian 12+ 作为首轮回归验证基线（非唯一支持发行版）。  
...
1. 默认采用用户目录内的隔离配置与运行状态目录（例如 `~/.local/share/vcpinstallergui`）。
2. 默认流程不得修改系统级 PATH、全局 npm/pip 包或系统服务配置，除非用户显式选择系统集成模式。
3. 安装器生成的运行状态文件（配置、PID、日志）应可通过 `reset`/清理流程回收。
4. 私有运行时目录必须支持整目录移除与重建，不残留系统级副作用。
...
10. 默认交付策略已确认：初始化下载私有运行时，不做全局安装。
...
10. 在 Linux `x86_64`（`glibc` 满足基线）环境下，默认最小配置路径可完成一键安装闭环；Ubuntu 22.04+ / Debian 12+ 需通过回归验证。
11. 遇到系统级缺失依赖时，安装器需输出可执行修复指引，并支持修复后 `resume` 继续。
12. 在 Desktop 模式下，首次运行可完成初始化向导并在同次流程启动后端与 VCPChat。
13. 首次初始化完成后，后续再次运行安装器默认进入一键启动，不重复初始化向导。
14. 用户可通过 `status` 查看组件运行状态，并通过 `stop` 停止已启动组件。
15. 在默认模式下，`init` 完成后可生成并校验私有运行时清单，失败时能 `resume` 继续。
16. 默认模式安装完成后，系统全局 PATH 与全局 npm/pip 包不发生变更。
17. 用户删除安装器隔离目录后，可完全回收运行时与缓存文件。
...
### 10.2 本轮评审决策（2026-02-25）

1. 发行版承诺采用最小口径：首发兼容声明限定为 Ubuntu 22.04+ / Debian 12+。
2. 离线路径采用“用户自备离线包 + 安装器导入”，安装器提供下载跳转入口，不负责自动预置离线包。
3. `config.env` 修改策略采用方案 A：默认不改已有 `config.env`；仅在文件不存在时生成，或用户显式 `--overwrite-config` 时重建。
4. 安装完成后提供运行选择框，由用户决定是否立即启动后端与 VCPChat。
5. GUI 必需步骤判定规则当前采用 A（固定白名单）并输出结构化阻塞信息，后续演进到 C（插件声明+核心兜底）。
6. 首发高级配置范围采用“全部字段”，并保留最小配置默认入口与“一键恢复最小配置”兜底。

评审展开文档：`docs/review-open-items.md`（含选项、利弊、建议与决策模板）。

### 10.3 已确认决策

1. 默认安装策略采用 Portable 模式：初始化阶段下载私有运行时，不安装到系统全局。
2. 初始化阶段下载运行时与依赖包可行，失败时阻断并通过 `resume` 恢复，不允许静默降级为全局安装。
3. 平台口径采用 Linux `x86_64`（`glibc` 基线），Ubuntu/Debian 仅作为回归验证基线。
4. Gitee 镜像发布策略暂时冻结，不作为当前阶段阻塞项。

---

## 11. 下一步

1. 按冻结需求补齐后续设计文档：架构设计、任务编排、配置模型、验收测试计划。
2. 落地离线包导入入口与下载跳转交互。
3. 落地安装后运行选择框与高级配置“全字段”界面方案。
4. 将 `config.env` 处理逻辑与“默认不改已有配置”的冻结需求口径对齐。
